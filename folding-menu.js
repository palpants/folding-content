...
MIT License
*/
var FOLDINGMENU = FOLDINGMENU || (function() {
  var _args = {};
  ...
  return{
    init : function( Args ) {
      jQuery( document ).ready( function() {
        ...
        _args = Args;
        var menuSelector          = _args[0];
        var menuItemSelector      = _args[1];
        var menuItemLink          = menuItemSelector + ' > a';
        var contentSelector       = _args[2];
        var unfoldedContentBefore = _args[3];
        var unfoldedContentAfter  = _args[4];
        var closeButtonMarkup     = '';
        if ( _args[5] ) {
          closeButtonMarkup       = _args[5];
        }

        // A bunch more class name conflict catches should go here
        currentClasses = [ menuSelector, menuItemSelector, contentSelector ];
        if ( jQuery.inArray( 'unfolded-content', currentClasses ) > -1 ){
        ...
        jQuery( menuItemSelector ).each( function() {
        ...
        function activeItemRow( $foldingActiveItem ) {
          ...
          while( false === endPosition ) {
            ...
            //console.log( '--- ' + rowEndNumber + ' ' + numberInThisRow + ' ' + widthCounter + ' ' + endPosition );
          } // while
